...
static void set_pte_address(page_table_entry_t* entry, uint64_t addr)
{
    addr >>= 12;
...
static uint64_t get_pte_address(page_table_entry_t* entry)
{
    uint64_t addr;
    addr = entry->addr_hi;
    addr <<= 32;
...
static uint64_t paging_map_temporary_page(uint64_t paddr, uint8_t allow_writes, uint8_t allow_user_access)
{
...
static void paging_unmap_temporary_page(uint64_t vaddr)
{
    uint64_t index;
...
{
    current_pml4 = (page_table_t) VADDR_GET_TEMPORARY(0);
...
{
    return pml4_map_memory(current_pml4, paddr, vaddr, size, 1, 1);
}
...
{
    uint64_t pd_idx = VADDR_TO_PML4_IDX(vaddr);
    page_table_t pt;
    ...
    while (total_unmapped_size < size && pd_idx < MAX_TABLE_ENTRIES)
    {
        ...
        unmapped_size = pt_unmap_memory(pt, vaddr, size);
...
{
    uint64_t pdp_idx = VADDR_TO_PML4_IDX(vaddr);
    page_table_t pd;
    page_table_entry_t entry;
    uint64_t i;
    ...
    while (total_unmapped_size < size && pdp_idx < MAX_TABLE_ENTRIES)
    {
        ...
        unmapped_size = pd_unmap_memory(pd, vaddr, size);
...
)
{
    ...
    while (total_unmapped_size < size && pml4_idx < MAX_TABLE_ENTRIES)
    {
        ...
        unmapped_size = pdp_unmap_memory(pdp, vaddr, size);
...
)
{
    ...
    while (mapped_size < size && pd_idx < MAX_TABLE_ENTRIES)
    {
        entry = pd[pd_idx];
        ...
        pt = (page_table_t) pt_vaddr;
        mapped_size = pt_map_memory(pt, paddr, vaddr, size, allow_writes, allow_user_access);
...
)
{
    ...
    while (mapped_size < size && pdp_idx < MAX_TABLE_ENTRIES)
    {
        entry = pdp[pdp_idx];
        ...
        pd = (page_table_t) pd_vaddr;
        mapped_size = pd_map_memory(pd, paddr, vaddr, size, allow_writes, allow_user_access);
...
)
{
    ...
    while (mapped_size < size && pml4_idx < MAX_TABLE_ENTRIES)
    {
        entry = pml4[pml4_idx];
        ...
        pdp = (page_table_t) pdp_vaddr;
        mapped_size = pdp_map_memory(pdp, paddr, vaddr, size, allow_writes, allow_user_access);
